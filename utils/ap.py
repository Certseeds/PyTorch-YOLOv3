...
from utils.utils import yolo_to_coco
...
def file_labels(ground_truths: List[List], predruths: List[List]) -> Tuple[List[Tuple[int, float]], int]:
    global x
    predList: List[Tuple[int, float]] = list()
    for predruth in predruths:
        bb = geter.filter_by_threshold(predruth, ground_truths)
        predList.append((bb + x, predruth[4]))
        print(bb + x)
    x += len(ground_truths)
    return predList, len(ground_truths)
...
def get_dataset_ap(dataset, detect_labespath: str):
    ...
    for i in range(0, annos, 1):
        filepath, img, ground_truth = dataset[i]
        ground_truth = ground_truth[:, 2:]  # shape of ground_truth[0] is tuple(int*6),so should delete the zeros before
        ground_truth = yolo_to_coco(ground_truth)
        # By img.shape
